  ...
  <div class="card">
    ...
    >
      ...
      <Column field="name" header="Name" sortable style="min-width: 14rem">
        <template #body="{ data }">
          <router-link :to="{ name: 'PatientDetail', params: { id: data.id } }">
            {{ data.name }}
          </router-link>
        </template>
      ...
      <Column field="doctor" header="Doctor" sortable style="min-width: 14rem">
        <template #body="{ data }">
          {{ data.doctor }}
        </template>
        <template #filter="{ filterModel }">
      ...
      <Column field="result" header="Result" sortable style="min-width: 12rem">
        <template #body="{ data }">
          <Tag :value="data.result" :severity="getSeverity(data.result)" />
        </template>
        <template #filter="{ filterModel }">
          <Select
            v-model="filterModel.value"
...
onMounted(async () => {
  try {
    ...
    if (response.data) {
      patients.value = response.data; // Directly assign the array to patients
